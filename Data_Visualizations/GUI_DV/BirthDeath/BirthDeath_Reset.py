...
sqlLocation = '../../AllData/SQL_BirthDeath_Rate.sqlite'
START_YEARS = 1960
LATEST_YEARS = 2015
...
def returnSQLQuery(rowVal, bdVal, sqlCalls, suffix):
    ...
    returnSQLQuery = """INSERT INTO %s%s (%s%s)
                        VALUES (%s)""" % (bdVal, suffix, "Country", sqlCalls, joinSql)
...
commands = ["drop table if exists birth_rate;",
            "drop table if exists death_rate;",
            
    startValues("birth", SUFFIX_VAL, sqlCreate),
    startValues("death", SUFFIX_VAL, sqlCreate)]
...
for row in csv.reader(f):
    ...
    if row[2] == "Birth rate, crude (per 1,000 people)":
        sqlQuery = returnSQLQuery(row, "birth", sqlCall, SUFFIX_VAL)
    elif row[2] == "Death rate, crude (per 1,000 people)":
        sqlQuery = returnSQLQuery(row, "death", sqlCall, SUFFIX_VAL)
